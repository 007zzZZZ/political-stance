...
def main():
    ...
    # 特定注入
    for topic in DEFAULT_TOPICS:
        spec_coords = build_coords(os.path.join(base_dir, topic, 'specific_neurons.json'))
        res = generate_with_patching(base_model, aligned, tokenizer,
                                     prompt_template, spec_coords, gen_kwargs,
                                     topic)
        out_path = os.path.join(base_dir, topic, 'specific_injected.json')
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        json.dump(res, open(out_path, 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
